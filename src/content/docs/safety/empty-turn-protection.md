...
If history protection failed silently (the model received a malformed history and produced strange output), the user would see strange output. They might also infer something about their application that is not true. The reconciliation pass still prevents an unauthorised state change, but the customer experience could include misleading text. That is the kind of conduct outcome the [FCA Consumer Duty](https://www.fca.org.uk/publications/policy-statements/ps22-9-new-consumer-duty) framework asks us to avoid: communications that are not clear, not fair, and not non-misleading. See [regulatory/consumer-duty](/regulatory/consumer-duty) for the underlying rule and [PRIN 2A](https://www.handbook.fca.org.uk/handbook/PRIN/2A.html) for the Handbook text.
